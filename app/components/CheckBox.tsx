...
export default function CheckBox( { label, ...props }: Checkboxprops) {
    ...
    return (
        <div className='flex flex-row space-x-2 items-center cursor-pointer'>
            <label htmlFor={props.id} className='bg-white w-10 md:w-11 h-6 rounded-full border-indigo-800 border-2 flex items-center px-px py-py'>
                <input type='checkbox' className='sr-only peer' {...props}></input>
            ...
            <label htmlFor={props.id} className='text-black font-bold text-xs'>{ label }</label>
